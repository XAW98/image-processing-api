...
describe('Test responses from endpoints', (): void => {
  ...
  describe('endpoint: /api/images', (): void => {
    it('gets /api/images?file=fjord (valid args)', async (): Promise<void> => {
      const response: supertest.Response = await request.get(
        '/api/images?file=fjord'
      );

      expect(response.status).toBe(200);
    });

    it('gets /api/images?file=fjord&width=199&height=199 (valid args)', async (): Promise<void> => {
      const response: supertest.Response = await request.get(
        '/api/images?file=fjord&width=199&height=199'
      );

      expect(response.status).toBe(200);
    });

    it('gets /api/images?file=fjord&width=-200&height=200 (invalid args)', async (): Promise<void> => {
      const response: supertest.Response = await request.get(
        '/api/images?file=fjord&width=-200&height=200'
      );
